...
export default function ReportPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const zustandLead = leads.find((lead) => lead.id === id)
  ...
  if (!lead) {
    return (
      <div className="theme-light doctor-shell min-h-screen flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          <div className="w-8 h-8 rounded-full border-2 border-[rgba(196,163,90,0.18)] border-t-[#C4A35A] animate-spin" />
          <p className="font-body text-[12px] text-[rgba(26,26,46,0.45)]">Rapor yükleniyor...</p>
        </div>
      </div>
    )
  }
  ...
  const radarAnalysis = lead.radar_analysis as
    | { radarScores?: Array<{ key: string; label: string; score: number; confidence?: number }> }
    | undefined
  const wrinkleScores = lead.wrinkle_scores as
    | { regions?: Array<{ region: string; label: string; score: number; level?: string }>; overallScore?: number }
    | undefined
  const overallScore =
  ...
  return (
    <div className="theme-light doctor-shell min-h-screen print:bg-white">
      <div className="print:hidden sticky top-0 z-10 doctor-card-strong border-x-0 border-t-0 rounded-none px-6 py-3 flex items-center justify-between">
        <button
          onClick={() => router.back()}
          className="font-body text-[12px] text-[rgba(26,26,46,0.55)] hover:text-[#C4A35A] transition-colors"
        >
          ← Geri Dön
        </button>
        <button
      ...
      <div className="max-w-[840px] mx-auto px-8 py-10 print:px-0 print:py-6 print:max-w-none">
        <div className="flex justify-between items-start mb-8 pb-6 border-b-2 border-[rgba(196,163,90,0.12)] print:border-[#1A1A2E]">
          ...
          <div className="text-right">
            <p className="font-body text-[11px] text-[rgba(26,26,46,0.50)] print:text-[rgba(0,0,0,0.5)]">Rapor Tarihi</p>
            <p className="font-body text-[13px] text-[#1A1A2E]">{formatDate(new Date().toISOString())}</p>
            <p className="font-mono text-[10px] text-[rgba(26,26,46,0.35)] mt-1 print:text-[rgba(0,0,0,0.4)]">ID: {lead.id.slice(0, 12)}</p>
          </div>
        </div>

        {overallScore != null && (
          <div className="doctor-card-strong mb-8 rounded-xl p-6 flex items-center gap-6 print:border-[rgba(0,0,0,0.1)] print:bg-[rgba(0,0,0,0.02)]">
            <div className="flex-shrink-0 w-16 h-16 relative">
              <svg viewBox="0 0 64 64" className="w-full h-full -rotate-90">
                <circle cx="32" cy="32" r="28" fill="none" stroke="rgba(26,26,46,0.10)" strokeWidth="3" className="print:stroke-[rgba(0,0,0,0.08)]" />
                <circle
                  cx="32"
                  cy="32"
                  r="28"
            ...
            <div>
              ...
              <p className="font-body text-[13px] text-[rgba(26,26,46,0.70)] print:text-[rgba(0,0,0,0.6)]">
                {overallScore >= 70
                  ...
                  : overallScore >= 40
                    ? 'Orta seviye bulgular tespit edildi.'
        ...
        <section className="mb-8">
          <SectionTitle>Hasta Bilgileri</SectionTitle>
                ...
                <ul className="space-y-1.5">
                  {aiScores.suggestions.map((suggestion: string, index: number) => (
                    <li key={index} className="font-body text-[12px] text-[rgba(26,26,46,0.72)] leading-relaxed flex gap-2 print:text-[rgba(0,0,0,0.7)]">
                      <span className="text-[#C4A35A] flex-shrink-0 print:text-[rgba(0,0,0,0.35)]">·</span>
                      {suggestion}
                    </li>
                  ))}
                </ul>
        ...
        {radarAnalysis?.radarScores && radarAnalysis.radarScores.length > 0 && (
          <section className="mb-8">
            <SectionTitle>Radar Analiz Skorları</SectionTitle>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2">
              {radarAnalysis.radarScores.map((region) => (
                <div key={region.key} className="flex items-center justify-between gap-3 py-1.5 border-b border-[rgba(196,163,90,0.08)] print:border-[rgba(0,0,0,0.06)]">
                  <span className="font-body text-[11px] text-[rgba(26,26,46,0.72)] print:text-[rgba(0,0,0,0.6)]">{region.label}</span>
                  <div className="flex items-center gap-2">
                    <div className="w-16 h-1.5 rounded-full bg-[rgba(26,26,46,0.08)] overflow-hidden print:bg-[rgba(0,0,0,0.06)]">
                      <div className="h-full rounded-full" style={{ width: `${region.score}%`, backgroundColor: scoreColor(region.score) }} />
                    </div>
                    <span className="font-mono text-[11px] w-6 text-right" style={{ color: scoreColor(region.score) }}>
                      {region.score}
                    </span>
                  </div>
                </div>
        ...
        {analysis && (
          <section className="mb-8">
            <SectionTitle>Bölgesel Değerlendirme</SectionTitle>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {[
                { title: 'Alın', keys: upperFace },
                { title: 'Nazolabial', keys: midFace },
                { title: 'Dudak', keys: lowerFace },
                { title: 'Genel', keys: general },
              ].map(({ title, keys }) => (
                <div key={title}>
                  <p className="font-body text-[11px] font-medium text-[rgba(26,26,46,0.72)] mb-2 print:text-[rgba(0,0,0,0.55)]">{title}</p>
                  <div className="space-y-2">
                    {keys
                      .filter((key) => analysis.region_scores[key] != null)
                      .map((key) => {
                        const regionScore = analysis.region_scores[key]
                        return (
                          <div key={key} className="flex items-center justify-between gap-3">
                            <span className="font-body text-[11px] text-[rgba(26,26,46,0.56)] capitalize print:text-[rgba(0,0,0,0.5)]">
                              {key.replace(/_/g, ' ')}
                            </span>
                            <div className="flex items-center gap-2">
                              <div className="w-16 h-1.5 rounded-full bg-[rgba(26,26,46,0.08)] overflow-hidden print:bg-[rgba(0,0,0,0.06)]">
                                <div className="h-full rounded-full" style={{ width: `${regionScore}%`, backgroundColor: scoreColor(regionScore) }} />
                              </div>
                              <span className="font-mono text-[11px] w-6 text-right" style={{ color: scoreColor(regionScore) }}>
                                {regionScore}
                              </span>
                            </div>
                          </div>
        ...
        {readiness && (
          <section className="mb-8">
            <SectionTitle>Konsültasyon Hazırlığı</SectionTitle>
            <div className="flex items-center gap-6 mb-4">
              ...
              <div>
                ...
                <p className="font-body text-[12px] text-[rgba(26,26,46,0.72)] print:text-[rgba(0,0,0,0.5)]">{readinessConfig?.action}</p>
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-0">
        ...
        {lead.doctor_notes && (
          <section className="mb-8">
            <SectionTitle>Doktor Notları</SectionTitle>
            <div className="doctor-card-soft p-4 rounded-xl print:bg-[rgba(0,0,0,0.02)] print:border-[rgba(0,0,0,0.08)]">
              <p className="font-body text-[12px] text-[rgba(26,26,46,0.72)] leading-relaxed whitespace-pre-wrap print:text-[rgba(0,0,0,0.7)]">{lead.doctor_notes}</p>
              {lead.doctor_notes_updated_at && (
                <p className="font-mono text-[10px] text-[rgba(26,26,46,0.35)] mt-2 print:text-[rgba(0,0,0,0.4)]">
                  {formatDateTime(lead.doctor_notes_updated_at)}
                </p>
              )}
            </div>
          </section>
        ...
        <div className="mt-12 pt-6 border-t border-[rgba(196,163,90,0.10)] print:border-[rgba(0,0,0,0.1)]">
          <div className="flex justify-between items-end">
            <div>
              <p className="font-body text-[10px] text-[rgba(26,26,46,0.40)] leading-relaxed print:text-[rgba(0,0,0,0.4)]">
                Bu rapor AI destekli ön değerlendirme çıktısıdır.
                <br />
                Kesin tedavi planı klinik muayene ve doktor değerlendirmesi sonrasında oluşturulur.
              </p>
            </div>
            <div className="text-right">
              <p className="font-body text-[10px] text-[rgba(26,26,46,0.40)] print:text-[rgba(0,0,0,0.4)]">Dr. Müjde Ocak Aesthetic Clinic · AI</p>
              <p className="font-mono text-[9px] text-[rgba(26,26,46,0.35)] print:text-[rgba(0,0,0,0.3)]">v1.0.0 · {formatDate(new Date().toISOString())}</p>
            </div>
          </div>
        </div>
      </div>
